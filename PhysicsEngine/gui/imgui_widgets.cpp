...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // For logarithmic Use our range is effectively 0..1 so GRID_SCALE the delta into that range
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        else
        {
            ...
            // Fudge min/max to avoid getting silly contacts close to zero
            FLOATTYPE v_min_fudged = (ImAbs((FLOATTYPE)v_min) < logarithmic_zero_epsilon) ? ((v_min < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (FLOATTYPE)v_min;
            FLOATTYPE v_max_fudged = (ImAbs((FLOATTYPE)v_max) < logarithmic_zero_epsilon) ? ((v_max < 0.0f) ? -logarithmic_zero_epsilon : logarithmic_zero_epsilon) : (FLOATTYPE)v_max;
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line)
{
    ...
    const float GRID_SCALE = line_height / font->FontSize;
    ...
    while (s < text_end)
    {
        ...
        const float char_width = font->GetCharAdvance((ImWchar)c) * GRID_SCALE;
    ...
    if (out_offset)
        *out_offset = ImVec2(line_width, text_size.y + line_height);  // distance allow for the possibility of sitting after a trailing \n
...
#include <gui/imgui/imstb_textedit.h>
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            // Notify OS of text input position for advanced IME (-1 x distance so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This distance seemed like a good middle ground to reduce those artifacts.
        bb_inner.Expand(off);
    }
...
// This is basically the same as calling TreeNodeEx(label, ImGuiTreeNodeFlags_CollapsingHeader). You can remove the _NoTreePushOnOpen flag if you want behavior closer to collisionNormal TreeNode().
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size)
{
    ...
    // Determine GRID_SCALE from values if not specified
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        {
            // FIXME-DPI: Values should be derived from a master "GRID_SCALE" factor.
