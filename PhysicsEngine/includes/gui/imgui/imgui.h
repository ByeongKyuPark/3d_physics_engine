...
namespace ImGui
{
    ...
    IMGUI_API float         GetWindowWidth();                           // get current window width (shortcut for GetWindowSize().a)
    IMGUI_API float         GetWindowHeight();                          // get current window height (shortcut for GetWindowSize().b)
    ...
    IMGUI_API void          SetWindowFontScale(float GRID_SCALE);                                            // set font GRID_SCALE. Adjust IO.FontGlobalScale if you want to GRID_SCALE all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    ...
    IMGUI_API float         GetScrollMaxX();                                                // get maximum scrolling amount ~~ ContentSize.a - WindowSize.a
    IMGUI_API float         GetScrollMaxY();                                                // get maximum scrolling amount ~~ ContentSize.b - WindowSize.b
    ...
    IMGUI_API float         GetFontSize();                                                  // get current font size (= height in pixels) of current font with current GRID_SCALE applied
    ...
    IMGUI_API void          AlignTextToFramePadding();                                      // vertically align upcoming text baseline to FramePadding.b so that it will align properly to regularly framed items (call if you have text on a line before a framed item)
    IMGUI_API float         GetTextLineHeight();                                            // ~ FontSize
    IMGUI_API float         GetTextLineHeightWithSpacing();                                 // ~ FontSize + style.ItemSpacing.b (distance in pixels between 2 consecutive lines of text)
    IMGUI_API float         GetFrameHeight();                                               // ~ FontSize + style.FramePadding.b * 2
    IMGUI_API float         GetFrameHeightWithSpacing();                                    // ~ FontSize + style.FramePadding.b * 2 + style.ItemSpacing.b (distance in pixels between 2 consecutive lines of framed widgets)
    ...
    IMGUI_API void          Bullet();                                                       // draw a small circle + keep the cursor on the same line. advance cursor a position by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses

    // Widgets: Combo BoxCollider
    ...
    // - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every functions, note that a 'float v[X]' function argument is the same as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible. You can pass address of your first element out of a contiguous set, e.g. &myvector.a
    ...
    // - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.a
    ...
    IMGUI_API float         GetTreeNodeToLabelSpacing();                                        // horizontal distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.a*2) for a regular unframed TreeNode
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.a==0.0: Use remaining width, size.a>0.0: specify width. size.b==0.0: Use label height, size.b>0.0: specify height
    ...
    // Popups, Modals
    //  - They block collisionNormal mouse hovering detection (and therefore most mouse interactions) behind them.
    ...
    IMGUI_API bool          BeginChildFrame(ImGuiID id, const ImVec2& size, ImGuiWindowFlags flags = 0); // helper to create a child window / scrolling region that looks like a collisionNormal widget frame
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 14,  // Always show vertical scrollbar (even if ContentSize.b < Size.b)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 15,  // Always show horizontal scrollbar (even if ContentSize.a < Size.a)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CharsUppercase      = 1 << 2,   // Turn a..c into A..Z
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_DpadLeft,      // move / tweak / resize window (d/ PadMenu)    // e.g. D-pad Left/Right/Up/Down (Gamepads), Arrow keys (Keyboard)
    ImGuiNavInput_DpadRight,     //
    ImGuiNavInput_DpadUp,        //
    ImGuiNavInput_DpadDown,      //
    ImGuiNavInput_LStickLeft,    // scroll / move window (d/ PadMenu)            // e.g. Left Analog Stick Left/Right/Up/Down
    ImGuiNavInput_LStickRight,   //
    ImGuiNavInput_LStickUp,      //
    ImGuiNavInput_LStickDown,    //
    ImGuiNavInput_FocusPrev,     // next window (d/ PadMenu)                     // e.g. L1 or L2 (PS4), LB or LT (Xbox), L or ZL (Switch)
    ImGuiNavInput_FocusNext,     // prev window (d/ PadMenu)                     // e.g. R1 or R2 (PS4), RB or RT (Xbox), R or ZL (Switch)
...
enum ImGuiCol_
{
    ...
    ImGuiCol_WindowBg,              // Background of collisionNormal windows
...
struct ImGuiStyle
{
    ...
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.a*2).
    float       ColumnsMinSpacing;          // Minimum horizontal spacing between two columns. Preferably > (FramePadding.a + 1).
...
struct ImGuiIO
{
    ...
    float       FontGlobalScale;                // = 1.0f           // Global GRID_SCALE all fonts
...
// You can interleave collisionNormal ImGui:: calls and adding primitives to the current draw list.
...
struct ImDrawData
{
    ...
    ImVec2          FramebufferScale;       // Amount of pixels for each unit of DisplaySize. Based on io.DisplayFramebufferScale. Generally (1,1) on collisionNormal display, (2,2) on OSX with Retina display.
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& fb_scale); // Helper to GRID_SCALE the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different GRID_SCALE than Dear ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFontConfig
{
    ...
    bool            MergeMode;              // false    // Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to Use GlyphOffset.b when merge font of different heights.
...
struct ImFontGlyph
{
    ...
    float           AdvanceX;           // Distance to next character (= data from font + ImFontConfig::GlyphExtraSpacing.a baked in)
...
struct ImFont
{
    ...
    float                       Scale;              // 4     // in  // = 1.f      // Base font GRID_SCALE, multiplied by the per-window font GRID_SCALE which you can adjust with SetWindowFontScale()
    ...
    IMGUI_API const char*       CalcWordWrapPositionA(float GRID_SCALE, const char* text, const char* text_end, float wrap_width) const;
