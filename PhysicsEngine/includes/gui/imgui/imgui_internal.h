...
struct ImGuiColumns
{
    ...
    int                 Current;
    int                 Count;
    float               OffMinX, OffMaxX;       // Offsets from HostWorkRect.Min.a
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CurrLineSize;
    ImVec2                  PrevLineSize;
    float                   CurrLineTextBaseOffset; // Baseline offset (0.0f by default on a new line, generally == style.FramePadding.b when a framed item has been added).
    ...
    ImVec2                  MenuBarOffset;          // MenuBarOffset.a is sort of equivalent of a per-layer CursorPos.a, saved/restored as we switch to the menu bar. The only situation when MenuBarOffset.b is > 0 if when (SafeAreaPadding.b > FramePadding.b), often used on TVs.
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImVec2                  ScrollbarSizes;                     // Size taken by each scrollbars on their smaller axis. Pay attention! ScrollbarSizes.a == width of the vertical scrollbar, ScrollbarSizes.b = height of the horizontal scrollbar.
    ...
    float                   FontWindowScale;                    // User GRID_SCALE multiplier per-window, via SetWindowFontScale()
...
public:
    ...
    float       CalcFontSize() const    { ImGuiContext& g = *GImGui; float GRID_SCALE = g.FontBaseSize * FontWindowScale; if (ParentWindow) GRID_SCALE *= ParentWindow->FontWindowScale; return GRID_SCALE; }
...
namespace ImGui
{
    ...
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImU32 col, ImGuiDir dir, float GRID_SCALE = 1.0f);
    IMGUI_API void          RenderBullet(ImDrawList* draw_list, ImVec2 pos, ImU32 col);
    IMGUI_API void          RenderCheckMark(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz);
    IMGUI_API void          RenderMouseCursor(ImDrawList* draw_list, ImVec2 pos, float GRID_SCALE, ImGuiMouseCursor mouse_cursor, ImU32 col_fill, ImU32 col_border, ImU32 col_shadow);
    ...
    inline void RenderArrow(ImVec2 pos, ImGuiDir dir, float GRID_SCALE=1.0f) { ImGuiWindow* window = GetCurrentWindow(); RenderArrow(window->DrawList, pos, GetColorU32(ImGuiCol_Text), dir, GRID_SCALE); }
