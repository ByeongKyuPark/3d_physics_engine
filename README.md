...
8. [Acknowledgements and References](#8-acknowledgements-and-references)
...
- **RigidBody**: As evident in the `RigidObject` class, a rigid body in this physics engine contains properties such as mass, position, and velocity. It acts as the fundamental unit of simulation, responding to forces and participating in collisions.
...
- **Renderer**: This component is in charge of drawing the objects on the screen, translating the physical world's state into visuals.
...
## 5. How to Build
The project is configured using premake. To build it on Windows:
```.\premake5.exe vs2022```
...
- Sphere & Cube: Click to create standard objects.
...
- Modify physical attributes and textures of the selected object.
- Duplicate Objects: With an object selected, press "ctrl+d".

**Global Settings**:

- Adjust global variables like gravity and time step in the "Settings" tab.

**Scenario Management**:

- "Save & Load" tab for managing preset scenarios.
- Pause/Resume: Use GUI buttons or press space bar.
...
- Additional Panning: Drag with left mouse button.

**Object Interactions**:

- Objects have behaviors based on their attributes and settings.
- Example: Bomb spawns objects on impact.

**Matrix-Test Project**:

- Uses the Google Test framework.
- Validates accuracy of matrix multiplication in the engine.
...
- ImGui: Provides the graphical user interface.
